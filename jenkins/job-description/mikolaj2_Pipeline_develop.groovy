...
def job_config = [
        job: [
                name: "mikolaj2_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1670490193783"
        ]
]
...
def customLib = library identifier: 'mikolaj2_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/mikolaj2_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.mikolaj2.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "jarro06", "mikolaj2", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("mikolaj2_Pipeline_develop");

job.with {

    authenticationToken('TmiiPrP4wX62p3qZTQXdIK4DjjM7CS4T')
